...
int main(int argc, char const* argv[]) {
    ...
    std::cout << "Running serial (flat allocation) computation...\n";
    {
        // Pointers to flat allocation that is N*N elements
        Type* a = flat::allocateMx(N);
        Type* b = flat::allocateMx(N);
        Type* c = flat::allocateMx(N);
        ...
        auto start = std::chrono::high_resolution_clock::now(); // Start a timer
        flat::multiplyMx(a, b, c, N);                           // Execute matrix multiply
        auto end = std::chrono::high_resolution_clock::now();   // Top the timer
        ...
        // Print time to output
        std::cout << "N = " << N << ":  " << time << " ms\n";
        flat::outputFile(c, "validation.bin", N);          // Write output to file
        flat::validateAgainstFile(c, "validation.bin", N); // Compare output to validation file
        ...
        flat::deallocateMx(a, N);
        flat::deallocateMx(b, N);
        flat::deallocateMx(c, N);
    }
}
